...
if($_POST){
    ...
    if(empty($nama_user)){
        ...
        echo "<script>alert('role tidak boleh kosong');location.href='ubah_user.php';</script>";
    }else {
        include "koneksi.php";
        ...
        $update = mysqli_query($conn, "update user set nama_user='" . $nama_user . "', username='" . $username . "', password='" . md5($password) . "', role='" . $role . "' where id_user = '" . $id_user . "' ") or die(mysqli_error($conn));
            if($update){
                ...
                echo "<script>alert('Gagal update user');location.href='ubah_user.php?id_member=".$id_user."';</script>";
            }
